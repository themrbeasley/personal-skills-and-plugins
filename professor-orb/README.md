...
| forge-prompt | Skill | Image-generation prompt craft for FLUX.2 and ComfyUI: forge a new prompt, write an edit prompt against an approved image, or diagnose one that underperformed. Runs an iterative loop and saves to the campaign's `prompts/` directory | `/forge-prompt`, "write an image prompt," "edit this image," "fix this prompt" |
