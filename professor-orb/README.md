...
| prep | Skill | Builds a session brief with the DM: work review, recap, planned scenes, handout list, outstanding lore items | "plan next session," "session prep," "what do I need to prep" |
...
| chronicler | Skill | Drafts a knowledge base update proposal from the session report and executes it after DM approval. The only pipeline skill that writes KB articles | "update the lore," "canonize last session," "apply the session changes" |
...
| forge-prompt | Skill | Image-generation prompt craft for FLUX.2 and ComfyUI: forge a new prompt, write an edit prompt against an approved image, or diagnose one that underperformed. Resolves a style first, from any campaign's catalog or one built with the DM, then runs a three-block loop whose suggestions each add, edit, or delete text in the prompt and keep invention out of it, saves to the campaign's `prompts/` directory, and can record which style is the house style in `CLAUDE.md` on approval | `/forge-prompt`, "write an image prompt," "edit this image," "fix this prompt" |
...
| /log | Command | Commits the session-reports lane (`session-reports/<setting>/<campaign>/`): reports, prep briefs, recaps, handouts, prompts, and the `articles/` subdirectory of staged KB articles. Sets an unfinished report aside by name and commits the rest | `/log`, "commit the session report," "commit the recap" |
...
**Standalone components**, never part of pipeline state: `setup` (after the first install), `homebrew`, `timeline`, `forge-prompt`, `/catalog`, `/scribe`, `/log`, `/migrate`, and `/sweep`. These run on demand at any point regardless of where the pipeline stands, and none of them write pipeline state.
...
**The homebrew catalog records what is real at the table.** `/catalog` writes one type-specific entry per finalized piece of homebrew: its frontmatter stamps a lifecycle status (`playtest`, `active`, `reverted`, `discontinued`) and a version that carries the entry across revisions, while its body blocks preserve the DM's finalized content verbatim, never reformatted or completed. One block is different by design: Design Notes records why the artifact was designed the way it was, as labeled records each answering one design question about it, and is confirmed with the DM before the entry is written. The primary source is the DM's confirmed design, typically the `homebrew` skill's iterated output; an exported Foundry actor or item JSON the DM attaches is finalized source too, copied unchanged into `foundryvtt/<bucket>/` and rendered into the entry as markdown; a sheet from a system other than `dnd5e` keeps the entry layout and fills its stat block with that system's own fields. Catalog entries sit outside the wikilink graph (no cross-references written into them, none pointing at them), but each one is still listed in its owning homebrew index so it stays discoverable. Where the `homebrew` skill authored VTT import files for the artifact, `/catalog` names each one in its commit pathspec so they travel with the entry; those buckets hold other artifacts' files, so they are enumerated individually rather than added as a directory.
