...
setup must not assume Rolara's artifacts exist. The plugin ships no templates or
schemas.
...
(`rolara-kb/` in Rolara). Only the chronicler skill mutates it, and only after DM
approval.
...
**conventions file**:
The machine-checkable derivation of the consumer's conventions, stored in
`.professor-orb/` and consumed by the deterministic write-time validator hook.
Each rule carries its own enforcement level (`block` / `warn` / `off`); strictness
is per-rule, not global (e.g., NSFW→unpublished is block; tag usage is lenient).
Regenerated by the setup skill; never hand-edited as primary.
_Avoid_: treating it as the source of truth (it is derived)

**setup skill**:
One-time (plus resync) skill that produces the conventions file. Three intake
tiers: (1) an existing conventions doc like Rolara's KB-CONVENTIONS.md → translate;
(2) conventions scattered across CLAUDE.md/other files/articles → discover and
consolidate; (3) conventions only in the DM's head → interview + infer from sample
articles. In tiers 2–3 it also drafts a human-readable KB-CONVENTIONS.md so every
consumer ends up with the same two-layer architecture. Always confirms
interpretations and per-rule enforcement levels with the DM.
...
(no wikilinks in or out; mechanics layer, like DnDBeyond footnotes). Organized
with sub-indexes per the KB's existing INDEX conventions as it grows. Serves
...
violations; the rebuild step proposes regenerated index files (propose-then-
execute, like chronicler). The setup skill applies the same index system to new
consumers, whether they arrive with an established KB or none at all. The most
consumer-specific logic in the plugin: expect a Rolara-tested iteration before
