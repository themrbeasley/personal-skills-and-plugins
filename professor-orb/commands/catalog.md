...
```
git add -- ":(literal)<entry file path>" ":(literal)<index file path>" ":(literal)<VTT import file 1>" ":(literal)<VTT import file 2>" ...
git commit --only -m "<message>" -- ":(literal)<entry file path>" ":(literal)<index file path>" ":(literal)<VTT import file 1>" ":(literal)<VTT import file 2>" ...
```

**VTT import files.** Where the `homebrew` skill named its paths in this same conversation, use those. Otherwise, determine them: this entry's own filename stem, with a `.json` extension, in the bucket its `type` maps to (the homebrew skill's bucket table); and any file `git status --porcelain -- ":(literal)<homebrewRoot>/foundryvtt"` reports as uncommitted, confirmed with the DM by name before it joins this capture rather than assumed. Name each file individually in both pathspecs above. Enumerate them individually: those buckets hold files belonging to other artifacts, and a directory-wide add would carry those into this entry's commit.
