...
Call `applyPlan` (`workflows/migrate.mjs`'s apply mode) with the parsed plan and its options: `cwd`, `settings`, `baseRules`, and `"commit": false`, because Step 10 makes the single commit.
...
In both cases point at Step 5's snapshot hash and stop without committing. Resetting to it discards Step 6's preparation commit along with it, which holds nothing the DM needs kept, but `git reset --hard` alone is not the whole recovery. It restores tracked paths only, and Step 7 may have already created files that were never staged: a `create-index` this scope emitted, for one. Those survive the reset as untracked files, and an untracked leftover is exactly what makes the next `/migrate` run's Step 3 clean-tree check stop, and what makes `applyPlan` refuse `dirty-tree` on the rerun too, so the DM would be sent to a state the migration cannot start from.

Before you report the recovery, run `git status --porcelain` yourself, right now, before anything is reset. Step 3 verified the tree held nothing uncommitted when this run began, so every `??` entry you see was created by this run; `.professor-orb/proposals/` never appears among them, because it is git-ignored. Report two commands, in order, and name the untracked paths you found so the DM sees exactly what the second one removes before running it:

```
git -C [project] reset --hard [Step 5's snapshot hash]
git -C [project] clean -fd
```

Be plain that `git clean -fd` deletes every untracked, non-ignored file in the tree, not only the ones this run made; naming the paths first is what makes running it safe here, given Step 3's guarantee that nothing else was sitting untracked before this run started.
