...
- Tag-implies-path rules: where the article's `tags` include any tag named in the rule's `tags`, its path must contain a directory segment equal to `requiredSegment`. Both comparisons are case-insensitive, and the segment is matched whole rather than as a substring, so a folder named `nsfw-rumors` does not satisfy a `requiredSegment` of `nsfw`. This is the containment check behind the project's permission deny rule: a deny rule matches paths and not file contents, so excluded content is only protected while it sits in the named folder. An article failing this check is content the deny rule cannot cover, which makes it the most urgent finding in a sweep. Report it as needing judgment rather than mechanically fixable: the fix is a file move, which changes the KB's structure and is `/migrate`'s work, not a field edit
