...
const checkerPrompt = (shardFiles, shardIdx, setting, kbRoot, rulesJson, indexSuffix, centralOwnership) =>
  [
    ...
    '4b. Check every tagImpliesPath rule: if this file\'s tags include any tag named in params.tags, its path must contain a directory segment equal to params.requiredSegment. Compare both the tag and the segment case-insensitively, and match the segment as a whole path segment rather than as a substring, so a folder named "nsfw-rumors" does NOT satisfy a requiredSegment of "nsfw". Report a violation as needing judgment, never as mechanically fixable: the fix is to move the file, which is a structural change /migrate performs, not a field edit the fix phase can apply. State in the question that this article is excluded content sitting outside the folder the project\'s permission deny rule covers, so nothing is currently protecting it.',
