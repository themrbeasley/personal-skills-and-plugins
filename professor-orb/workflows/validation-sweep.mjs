...
const checkerPrompt = (shardFiles, shardIdx, kbRoot, rulesJson, indexSuffix) =>
  [
    ...
    '6. Check every content category rule, skipping wikilink and orphan checks for catalog entries per step 3: wikilinkPolicy (wikilinks are well formed; if requireExistingTarget is true and a target clearly does not exist anywhere plausible, flag it, but if you cannot confirm one way or the other because the target might live in a different shard, do not flag it), tagVocabulary (never block on this; only collect tag usage, and only add an informational finding if the rule\'s enforcement is not "off"), prohibitedPattern (the body or frontmatter, per appliesTo, does not contain the disallowed pattern, for example an em dash character).',
