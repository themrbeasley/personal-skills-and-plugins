...
| `tagImpliesPath` | `tags` (array of tag names that mark content as excluded), `requiredSegment` (string, the folder name excluded content must sit under) | If the article's `tags` include any value in `tags`, its path must contain a directory segment equal to `requiredSegment`; otherwise it fails. This is the containment check behind a path-scoped permission deny rule (see "Excluded content" below): a `deny` rule matches paths, never file contents, so there is no rule that can express "deny anything tagged NSFW". Putting excluded content in a named folder is what makes the block expressible, and this check is what catches an article that is not in one. Both the tag and the folder name compare case-insensitively, the first because the DM types the tag by hand and the second because Windows and macOS filesystems treat `NSFW/` and `nsfw/` as one folder. The segment is matched whole, never as a substring, so a folder named `nsfw-rumors` or a file named `Nsfw-Rumors.md` does not satisfy it: the deny rule would not cover either, and counting them as contained would report a leak as safe. A rule with an empty `tags` list or no `requiredSegment` is inert. The shipped base rule is `off` with no tags named, because the vocabulary is the DM's to choose |
