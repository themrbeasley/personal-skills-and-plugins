---
name: setup
description: "One-time (plus on-demand resync) onboarding workflow that puts version control under a campaign project, migrates it onto professor-orb's canonical layout behind a snapshot commit, and produces the .professor-orb/ artifacts every other professor-orb skill and hook depends on (conventions.json, versioning.json, per-setting tag registries, proposals/), plus copies of the plugin's workflows in .claude/workflows/. If the DM names any excluded content tags (for example NSFW), also provisions a path-scoped permission deny rule in .claude/settings.json so excluded content is actually blocked from Read, Edit, Grep, and Glob, not merely flagged. Use it when installing professor-orb into a new campaign project, when the DM asks to set up or configure the plugin, or when .professor-orb/ is missing, stale, or has drifted from the KB's actual conventions."
---
...
You are onboarding a D&D DM's campaign project onto professor-orb. This skill runs once per project, plus an on-demand resync later if the KB drifts. Nothing else in the plugin works reliably until `.professor-orb/` exists: the write-time validator hook degrades gracefully (stays silent) until this skill has produced `conventions.json`, and the Stop hook finds each campaign's pipeline state through the `settings` array this skill writes into `conventions.json`. Treat this run as foundational, not a formality.
...
**Detecting drift never relies on comparing `schemaVersion` alone.** That field can be stale on the plugin's own side: a rule can be added to `references/base-rules.json` without its `schemaVersion` being bumped, in which case an equal `schemaVersion` on both sides reports no drift even though the base set has moved on. Diff the rule IDs in the project's `conventions.json` (per setting) against the rule IDs in `references/base-rules.json` directly. Any base rule ID present in `references/base-rules.json` with no counterpart in the project's file is drift, regardless of what `schemaVersion` says; name the specific missing rules when telling the DM the KB has drifted, so they can decide whether to resync now.
...
**Ask about content exclusions, in every tier, not only tier 3.** Several skills already carry an instruction to check article metadata for excluded tags before reading or citing an unfamiliar article, but until `frontmatterExcludedTagLocation` exists that instruction has nothing to enforce it. Ask the DM whether any tag or category should be walled off entirely (NSFW is the common case, but do not assume it is the only one, and do not assume every project has one). If they name any:

- Set `frontmatterExcludedTagLocation.params.tags` to the named list and `params.requiredSegment` to the folder name content under that tag must live in (propose `nsfw`, but let the DM choose). Set `enforcement` to `block`; this is not a rule to propose `warn` on, since a warn-level containment check leaves excluded content readable while only complaining about it after the fact.
- Say plainly what actually enforces this: a path-scoped `deny` rule in `.claude/settings.json`, provisioned at Step 12, blocks Read, Edit, Grep, Glob, and recognized Bash read commands against the named folder before any tool call runs. The `frontmatterExcludedTagLocation` rule is drift detection, not the wall itself: it catches a tagged article that ends up outside the folder the deny rule covers, where nothing else is protecting it.
- Say plainly what does not: a script Claude writes and executes can still open a file in that folder directly, on any platform without the sandbox enabled, which is native Windows. Do not round this up to an unqualified guarantee.
- If the KB already has articles carrying the named tag, they are not yet inside the required folder (setup does not move them; see below), so the DM should expect `/sweep` to report them and plan to run `/migrate` afterward.

If the DM names no excluded tags, leave `frontmatterExcludedTagLocation` at its shipped `enforcement: "off"` with an empty `tags` list, and provision no deny rule at Step 12. Do not invent a tag the DM did not name.

**Setup never moves content into the required folder.** Naming a tag and a folder here declares the convention; it does not relocate any article that predates it. That relocation is `/migrate`'s work, run by the DM, never setup's.
...
- **The excluded-content deny rule in `.claude/settings.json`**, only if Step 11 named at least one excluded tag. Read the file first if it exists; a fresh install or a DM's own permission rules may already be there, and this step adds to `permissions.deny`, it never replaces the file or the array. For each distinct `requiredSegment` named across every setting's `frontmatterExcludedTagLocation` rule, ensure both `"Read(**/<segment>/**)"` and `"Edit(**/<segment>/**)"` are present in `permissions.deny`; skip a pattern already there rather than duplicating it. Do not write a `Write(...)`, `Glob(...)`, or `NotebookEdit(...)` path rule: Claude Code accepts but never consults those, and `Edit(...)` already covers every file-editing tool. If `.claude/settings.json` does not exist, create it holding only `{ "permissions": { "deny": [...] } }`; do not add unrelated keys. On a resync, union with what is already there by the same skip-if-present rule; never remove an entry this step did not add.

Then delete `.professor-orb/conventions.json.pre-migration`, a legacy `.professor-orb/pipeline-state.json` (with `git rm` if it is tracked), a legacy `.professor-orb/asked-options.json` if one exists (with `git rm` if it is tracked), and, if Step 4 converted it, the old `.professor-orb/catalog-versioning.json`. The snapshot captured each of them. The legacy `pipeline-state.json` is the one shared breadcrumb from before 1.20.0: it names no campaign, so it cannot be converted, and each campaign's pipeline skills now write their own in `<sessionReportsRoot>/<campaign>/`. `versioning.json` itself is never regenerated here: Step 4 owns it, and `decided` is never rewritten.
