...
This skill is **standalone**, like `homebrew`, `timeline`, and `/catalog`. It is not part of the debrief, prep, content, chronicler, kb-validator session pipeline and never writes pipeline state.
...
Whichever row resolves a style, the offers in "Recording the style" below come next, before the first Revised Prompt.
...
- **Never write pipeline state.** This skill is standalone.
...
- **Never save mid-loop.** The style offers are settled in Step 1; the prompt is approved at the close.
