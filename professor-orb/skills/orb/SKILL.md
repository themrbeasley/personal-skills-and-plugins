...
| /log | Command | Commit the session-reports lane (`session-reports/<setting>/<campaign>/`): reports, prep briefs, recaps, handouts, prompts. Sets an unfinished report aside by name and commits the rest | `/log`, "commit the session report," "commit the recap" |
