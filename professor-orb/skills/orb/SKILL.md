...
| chronicler | Skill | Draft a KB lore-update proposal from the session report and execute it after DM approval. The only pipeline skill that writes KB articles | "update the lore," "canonize last session," "apply the session changes" |
...
| /log | Command | Commit the session-reports lane (`session-reports/<setting>/<campaign>/`): reports, prep briefs, recaps, handouts, prompts, and the `articles/` subdirectory of staged KB articles. Sets an unfinished report aside by name and commits the rest | `/log`, "commit the session report," "commit the recap" |
...
| /migrate | Command | Restructure the KB to a DM-stated scope, or to setup's deferred items and the sweep's needs-judgment findings; writes a proposal the DM may edit, then executes exactly what the approved file says | `/migrate`, "clean up items/", "rename X to Y everywhere", "retire the Karsk campaign" |
