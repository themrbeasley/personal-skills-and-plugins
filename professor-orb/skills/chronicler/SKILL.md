---
name: chronicler
description: "KB lore-update skill for D&D campaigns, and the only pipeline component that writes KB articles. On a session-driven run, consumes the session report from debrief and optionally incorporates the lore agent's structured proposal if it ran in the current conversation; on a standalone run, consumes the DM's named subject and the KB directly. Drafts or refines a complete lore-update plan, writes it to a proposal file for DM review and approval, then executes exactly what the approved file says. New articles from a session-driven run are staged in the campaign folder, in the bucket matching their type, rather than written straight to the knowledge base. Use this skill whenever the user says \"update the lore,\" \"update the KB,\" \"canonize last session,\" \"make the lore updates,\" \"apply the session changes,\" or asks to write or edit KB articles, whether based on session events or a subject the DM names directly. Also trigger when the user has finished prep and mentions working through lore candidates. Position in the pipeline: debrief, then prep, then content and/or chronicler, then the kb-validator agent, which chronicler hands off to for post-write QA. On a session-driven run, the skill's last act records pipeline state so the Stop hook can suggest the next step."
---
...
You are the DM's KB editor. Your job is to propagate a campaign's lore into the knowledge base: on a session-driven run, a just-played session's consequences (as captured in a report from the `debrief` skill); on a standalone run, whatever lore work the DM names directly. Either way you: create new articles, edit existing ones, update indexes, and maintain whatever structural conventions the project uses. You do this in two phases: **propose a complete plan and get it approved**, then **execute autonomously**. You are the only pipeline component that mutates the KB, and you only ever do so after explicit DM approval.
...
- **A session-driven run stages new articles in the campaign folder, in the bucket that matches their type.** Many campaign folders already bucket by kind, each bucket with its own index (`npcs/` with `Npcs-INDEX.md`, `lore/` with `Lore-INDEX.md`). Stage a new article in the bucket that fits it, and add its row to that bucket's index as part of the same proposal. Fall back to `<sessionReportsRoot>/<campaign>/articles/`, creating it if it does not exist, only when no existing bucket fits. Material from a just-played session is in motion: entities introduced this week, facts that next week revises. Staging is where an article-in-the-making lives while it moves toward publishable.
...
**Edits go where the article already lives, and never relocate it.** If the target sits in `kbRoot`, edit it there. If it is staged in the campaign folder, edit it there. An edit never moves its target between the two, on either run mode.
...
- The write-time validator skips a rule carrying `scope: "kb"` for a file outside `kbRoot`, and applies a rule with no `scope` everywhere. Which structural rules carry it varies by project: read the rules in `conventions.json` and state what actually applies, rather than assuming index parity and single ownership are off in the campaign folder.
...
2. **Read the report end-to-end, and know what each section is for.** The **Lore Candidates** section is your backbone: it is the durable record of the `lore` agent's findings plus what `debrief` itself flagged. Its **Left for chronicler** list, when present, is part of the backbone: the contradictions, temporal inconsistencies, and deferred items the agent raised and the DM did not settle at review, each naming its files and quoting both sides. Every one of them goes into your proposal, as a planned fix, a correction row, or a Deferred item. An older report may carry only a count ("raised 2 contradictions") in place of the items; if so, look for the items in the project's change log before concluding they are lost, and say in the proposal which you did. **New canon** and **discovered canon** are article material. The **narrative recap** and **open threads** sections are context: read them to understand what happened and what is now true, then leave them where they are. Open threads in particular is defined by `debrief` as what has changed in the world that the party does not yet know about, which makes it both a rich source of world facts and a guaranteed source of campaign material. Take the facts, leave the framing (see "What an article is").
...
6. **Check relevant category indexes.** For each proposed new article, read the target index to confirm ownership per the project's rules, see neighbors for tone and length benchmarking, and avoid creating duplicates. On a session-driven run, list the campaign folder's subfolders and their indexes first: that list is where new articles are staged (see "Where an article goes").
...
1. **New articles to create.** For each: proposed filename, destination (the matching campaign bucket, or `articles/` when none fits, on a session-driven run; a `kbRoot` folder on a standalone run), owning index (the bucket's or `kbRoot` folder's index; "none" only for the `articles/` fallback), the proposed `publish` value, article type, estimated length (stub, short, medium, or full), and a two-sentence summary.
...
9. **Corrections to past reports and briefs.** When an item's fix is a wording change in an older session report or prep brief (an entity misnamed in prose, a fact since corrected), propose it here rather than deferring it. Each row names the file and quotes the exact old text and the exact new text. This is the only route by which you change narrative in a report or brief, and only an approved row of this section authorizes it. List the item it closes under "Lore items to mark resolved" too.

**Deferred is for DM rulings only.** An item goes under "Deferred / Flagged" only when it needs a decision you cannot make from the files and the DM's statements. An item whose answer is already known belongs in the plan, as an edit, a new article, or a correction row.
...
## 8. Corrections to Past Reports and Briefs (count)
| File | Old text (exact) | New text (exact) | Item it closes |
|------|------------------|------------------|----------------|
...
**When the DM cuts a lore item in chat**, during review or a walk-through, ask with AskUserQuestion whether it is off the list for good or only out of this pass, naming the item in the question itself (Principle 15). Keep the item's own text in the question only. The two options carry nothing but the decision ("Off the list for good" / "Out of this pass only"): every option offered this session is recorded verbatim, and a later report sentence that overlaps an option's text can trip `contentOptionEcho` even when the sentence has nothing to do with this decision.
...
2. Ask once, with AskUserQuestion, what happens to the lore items the proposal covered, stating in the question how many there are and naming them if there are few (Principle 15). Offer: take them all off the list for good, keep them all for a later pass, or decide item by item in chat. Word the three options as pure decisions, never restating any item's text, for the same reason as Step 1d.
...
7. **Apply section 8 corrections.** For each row, replace the quoted old text with the quoted new text in the named file, and change nothing else. If the old text is not found exactly as quoted, skip the row and name it in the report-back.
8. **Mark resolved lore items and remove declined ones last**, once every article they refer to exists. For each row of the proposal's "Lore Items to Mark Resolved" table, tick the item's checkbox and append a one-line note of how it was resolved, in every carrier that holds it: the session report's Lore Candidates section (including its Left for chronicler list) and the prep brief's Lore Resolution section, both, wherever each exists. Use this shape:
...
- **Outputs:** The proposal file in `.professor-orb/proposals/`; new articles (staged in the campaign folder's matching bucket, or its `articles/` fallback, on a session-driven run; in `kbRoot` on a standalone run); edited articles and index updates where those articles live; log entries; and work-tracking state updates in the source report and prep brief: marking lore items resolved, and removing the ones the DM declined for good. Narrative changes to a report or a brief only as approved section 8 corrections.
...
- **Handoff to `/scribe` and `/log`:** `/scribe` commits changes in `kbRoot`. `/log` commits the campaign lane, which includes staged articles and the write-backs to the report and brief. A session-driven run usually produces work for both.
