# Monster Statistics by CR: 2014 DMG Reference

Source: Dungeon Master's Guide (2014), "Creating a Monster" chapter.

This table provides the baseline defensive and offensive statistics for monsters at each CR. Use it as a benchmark when building or evaluating homebrew monsters. A monster can exceed one column meaningfully, and that's a design choice. Exceeding two or more columns without a compensating weakness usually means the CR is underrated.

**Note:** These values are guidelines, not hard constraints. A high-AC monster might have lower HP to compensate, and vice versa. The table represents a balanced average.

---

## How to Use This Table

- **Proficiency Bonus (PB)**: Used for attack rolls, save DCs, and skill/saving throw proficiencies.
- **Armor Class**: Typical AC for a monster of this CR.
- **Hit Points**: The expected HP range. A monster can trade HP for AC or vice versa.
- **Attack Bonus**: Total bonus on attack rolls.
- **Save DC**: The DC for the monster's abilities that call for saving throws.
- **Damage/Round**: Average total damage output per round across all attacks. This is the most important offensive stat.

---

## The Table

| CR   | Prof Bonus | Armor Class | Hit Points  | Attack Bonus | Save DC | Damage/Round |
|------|-----------|-------------|-------------|-------------|---------|-------------|
| 0    | +2        | 13          | 1–6         | +3          | 13      | 0–1         |
| 1/8  | +2        | 13          | 7–35        | +3          | 13      | 2–3         |
| 1/4  | +2        | 13          | 36–49       | +3          | 13      | 4–5         |
| 1/2  | +2        | 13          | 50–70       | +3          | 13      | 6–8         |
| 1    | +2        | 13          | 71–85       | +3          | 13      | 9–14        |
| 2    | +2        | 13          | 86–100      | +3          | 13      | 15–20       |
| 3    | +2        | 13          | 101–115     | +4          | 13      | 21–26       |
| 4    | +2        | 14          | 116–130     | +5          | 14      | 27–32       |
| 5    | +3        | 15          | 131–145     | +6          | 15      | 33–38       |
| 6    | +3        | 15          | 146–160     | +6          | 15      | 39–44       |
| 7    | +3        | 15          | 161–175     | +6          | 15      | 45–50       |
| 8    | +3        | 16          | 176–190     | +7          | 16      | 51–56       |
| 9    | +4        | 16          | 191–205     | +7          | 16      | 57–62       |
| 10   | +4        | 17          | 206–220     | +7          | 16      | 63–68       |
| 11   | +4        | 17          | 221–235     | +8          | 17      | 69–74       |
| 12   | +4        | 17          | 236–250     | +8          | 17      | 75–80       |
| 13   | +5        | 18          | 251–265     | +8          | 18      | 81–86       |
| 14   | +5        | 18          | 266–280     | +8          | 18      | 87–92       |
| 15   | +5        | 18          | 281–295     | +8          | 18      | 93–98       |
| 16   | +5        | 18          | 296–310     | +9          | 18      | 99–104      |
| 17   | +6        | 19          | 311–325     | +10         | 19      | 105–110     |
| 18   | +6        | 19          | 326–340     | +10         | 19      | 111–116     |
| 19   | +6        | 19          | 341–355     | +10         | 19      | 117–122     |
| 20   | +6        | 19          | 356–400     | +10         | 19      | 123–140     |
| 21   | +7        | 19          | 401–445     | +11         | 20      | 141–158     |
| 22   | +7        | 19          | 446–490     | +11         | 20      | 159–176     |
| 23   | +7        | 19          | 491–535     | +11         | 20      | 177–194     |
| 24   | +7        | 19          | 536–580     | +12         | 21      | 195–212     |
| 25   | +8        | 19          | 581–625     | +12         | 21      | 213–230     |
| 26   | +8        | 19          | 626–670     | +12         | 21      | 231–248     |
| 27   | +8        | 19          | 671–715     | +12         | 22      | 249–266     |
| 28   | +8        | 19          | 716–760     | +13         | 22      | 267–284     |
| 29   | +9        | 19          | 761–805     | +13         | 22      | 285–302     |
| 30   | +9        | 19          | 806–850     | +14         | 23      | 303–320     |

---

## Important Caveats

- **CR is a guideline.** A well-designed monster can deviate meaningfully from these ranges if its other traits compensate.
- **Damage/Round is the most critical column.** An encounter becomes dangerous or trivial faster through DPR than through AC or HP.
- **Defensive CR vs. Offensive CR**: The 2014 DMG calculates CR by averaging the defensive CR (based on HP/AC) and offensive CR (based on DPR/attack bonus/save DC). A monster with very high HP but low damage may have a misleading final CR.
- **Legendary Actions and Resistances**: These push effective CR up meaningfully and are not fully captured in the raw stat ranges. Factor them in qualitatively.
- **Verify against the 2014 DMG** if precision matters. These values are drawn from training data and should be accurate, but the original source is authoritative.
